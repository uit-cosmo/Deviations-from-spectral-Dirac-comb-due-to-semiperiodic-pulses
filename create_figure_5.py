...
for control_parameter in [0.1, 0.2, 0.3, 0.4]:
    ...
    ax1.semilogy(f, Pxx, label=rf"$\alpha = {control_parameter}$")
    ...
    ax2.plot(tb, R, label=rf"$\alpha = {control_parameter}$")

t, R_an = calculate_R_an(0, 1, 0.2)

PSD = PSD_periodic_arrivals(2 * np.pi * f, td=1, gamma=0.2, Arms=1, Am=0, S=S)
ax1.semilogy(
    f, PSD, "--k", label=r"$S_{\widetilde{\Phi}}(f), \, \langle A \rangle = 0$"
)

ax2.plot(t, R_an, "--k", label=r"$R_{\widetilde{\Phi}}(t), \, \langle A \rangle = 0$")
