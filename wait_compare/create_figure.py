...
def cf_gamma(omega, tw, wrms):
    return (1 - 1.0j * omega * wrms**2 / tw) ** (-((tw / wrms) ** 2))


def cf_unif(omega, tw, wrms):
    cf = (
        np.exp(1.0j * omega * tw)
        * np.sin(np.sqrt(3) * wrms * omega)
        / (np.sqrt(3) * wrms * omega)
    )
    cf[omega == 0] = 1
    return cf
...
def psd_invgamma(wrms):
    ...
    D = np.load(dname)
    return D["omega"] / (2 * np.pi), spectrum_waiting_time_part(
        D["omega"], lambda o: D["cf"]
    )
...
for row in range(rows * columns):
    ...
    for cf, ls, lab in zip([cf_norm, cf_gamma], linestyle[2:], labels[2:]):
        if row == 1:
            ax[row].semilogy(
                f,
                spectrum_waiting_time_part(omega, lambda o: cf(o, 1, Wrms[row])),
                ls=ls,
                label=lab,
            )
        else:
...
for wrms in Wrms:
    ...
    for cf, label in zip([cf_gamma, cf_unif], ["gamma", "unif"]):
        diff = np.max(
            np.abs(
                spectrum_waiting_time_part(omega, lambda o: cf_norm(o, 1, wrms))
                - spectrum_waiting_time_part(omega, lambda o: cf(o, 1, wrms))
            )
            / spectrum_waiting_time_part(omega, lambda o: cf_norm(o, 1, wrms))
        )
