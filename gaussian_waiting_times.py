...
for i, sigma in enumerate([0.1]):  # , 0.4, 3.0]):
...
def spectra_analytical(omega, gamma, A_rms, A_mean, sigma):
    ...
    second_term = gamma**2 * A_mean**2 * I_2 * Lorentz_PSD(omega) * np.sinh(
        nu**2 * Omega**2 / 2
    ) / (
...
PSD = spectra_analytical(2 * np.pi * f, gamma=0.2, A_rms=1, A_mean=1, sigma=0.1)
